=== settle-woocommerce ===
Tags: woocommerce, payment, gateway
Requires at least: 4.0 or higher
Tested up to: 4.2.2
Stable tag: 0.5
License: MIT License
License URI: http://opensource.org/licenses/MIT
WC requires at least: 2.2
WC tested up to: 2.3.10

Provides a Settle payment gateway for WooCommerce.

== Description ==

Provides a Settle payment gateway for WooCommerce.

[youtube https://www.youtube.com/watch?v=oMzxN6S5-lM]

Only availble for Norwegian residents.

== Installation ==

= the hard way =

1. Download, Unzip and drop the extention on /wp-content/plugins/ directory,
1. As admistration, activate the plugin through the 'Plugins' menu in WordPress,


= the easy way =

1. As administrator, goto 'Plugins' then Click on 'Add New',
2. Search for 'settle-woocommerce' then Click on 'Install Now',
3. Wait for it to download, Unpack and to install,
4. Activate the plugin by Clicking on 'Activate Plugin'
5. Goto the WooCommerce->Settings->Checkout->Settle->Settings to get started.

[youtube https://www.youtube.com/watch?v=SeGFEo9SFv0]

== Frequently Asked Questions ==

= How to get an Settle Merchant account =

Sign up for an account at https://business.settle.eu/signup/ . Create a merchant and a merchant user (not only a legal entity).


= Where do I find the plugin Documentation & FAQ ? =

The user guide and frequently asked questions can be found at https://github.com/settle/settle-woocommerce


== Screenshots ==

== Changelog ==

= 0.5 =
* Fix for including tax on payment request text
* Fix for when scope is not ready when payment ok

= 0.4 =
* Fixed faulty html and added banner and icon

= 0.3 =
* Added Settle Express

= 0.2 =
* Fix for php 5.3
* Fix for apache removing Authorization header

= 0.1 =
* First early adopter release. Tested on WooCommerce


