...
if(!class_exists('Settle_Client')) {
    class Settle_Client
    {

        const SETTLE_PUB_KEY_PROD = '-----BEGIN PUBLIC KEY-----
...
-----END PUBLIC KEY-----';
        ...
        public function __construct($log,
            ...
            $this->log          = $log;
            $this->settle_server = $settle_server;
            $this->mid          = $mid;
            $this->uid          = $uid;
            ...
            $this->settle_public_key = null;
        }

        public function log( $message )
        {
            if (!empty( $this->log ) ) {
                $this->log->add('settle_client', $message);
            }
        }

        function get_settle_public_key()
        {
            if(is_null($this->settle_public_key)) {
                return self::SETTLE_PUB_KEY_PROD;
            }
            return $this->settle_public_key;
        }
        ...
        function payment_request($success_return_uri,
            ...
            $result = $this->settle_merchant_call(
        ...
        function payment_outcome($settle_tid)
        {
            $payload = '';
            $url =  $this->settle_server . "/merchant/v1/payment_request/" . $settle_tid . "/outcome/";
            $result = $this->settle_merchant_call('GET', $url, $payload, $this->mid, $this->uid, $this->priv_key, $this->sslverify);
            return $result;
        }

        function capture_payment($settle_tid)
        {
        ...
        function refund_payment($settle_tid, $settle_refund_id, $amount, $text)
        {
            $url = $this->settle_server . "/merchant/v1/payment_request/" . $settle_tid . "/";
            $method = 'PUT';
            $payload = array(
        ...
        function settle_merchant_call($method, $url, $payload, $mid, $uid, $priv_key, $sslverify)
        {
            ...
            $this->log('settle_merchant_call()  $url = ' . print_r($url, true));
            $this->log('settle_merchant_call()  $headers = ' . print_r($headers, true));
            $this->log('settle_merchant_call()  $payload = ' . print_r($payload, true));
            $this->log('settle_merchant_call()  $json_payload = ' . print_r($json_payload, true));
            $this->log('settle_merchant_call()  $json_payload = ' . $json_payload);
            ...
            $this->log('settle_merchant_call()  $result = ' . print_r($result, true));
            return $result;
        }
        ...
        public function buildSignatureMessage($requestMethod, $url, $headers)
        {
            ...
            $headerPairs = array();
            foreach ($settleHeaders as $key => $value) {
        ...
        public function valid_signature($method, $uri, $headers, $json_payload)
        {
            ...
            $key = $this->get_settle_public_key();
        ...
        public function request_headers()
        {
            if( function_exists('apache_request_headers') ) {
                $this->log('Settle request_headers() apache_request_headers() is defined.');
                return apache_request_headers();
            } else {
                $this->log('Settle  request_headers() apache_request_headers() is not defined. Using fallback');
